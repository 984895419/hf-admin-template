<template>
  <div v-if="showFields">
    ...
    <div v-for="item in showFields" :key="item.value">
        ...
        />
        <el-table-column
          v-if="item.value === 'secret'"
          prop="secret"
          :label="$t(conf.namespace + '.secret')"
          :width="item.width"
          min-width="130"
        />
        <el-table-column
          v-if="item.value === 'token'"
          prop="token"
          :label="$t(conf.namespace + '.token')"
          :width="item.width"
          min-width="130"
        />
        <el-table-column
          v-if="item.value === 'aesKey'"
          prop="aesKey"
          :label="$t(conf.namespace + '.aesKey')"
          :width="item.width"
          min-width="130"
        />
